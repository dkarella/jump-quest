...
pub fn handle_player_input(world: &mut World, inputs: &Inputs) {
  let mut force_jump = false;
  for key in inputs.keys_pressed.iter() {
    match key.as_str() {
  ...
  match world.player.state {
    PlayerState::Jumping => {
      if !force_jump {
        if let Some(tile) = world.tiles.iter().find(|tile| {
          world
            .player
            .bottom_sensor()
            .intersects(&tile.landing_sensor())
        }) {
